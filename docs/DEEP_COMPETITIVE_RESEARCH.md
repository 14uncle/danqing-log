...
|------|-------|-------------|-------------------|----------|
| **1GB 冷启动索引** | ~800ms | **60,000ms** | **7,000ms** | **584ms** ✅ |
| **1GB 热启动索引** | ~150ms | ~5,000ms | ~500ms | **88ms** ✅ |
| **1GB 热搜索** | ~150ms | ~200ms | ~500ms | **69ms** ✅ |
| **JSONL 字段过滤** | N/A | ~10,000ms | ~500ms | **74ms** ✅ |
| **10GB 文件打开** | ~3s | ~600s | ~70s | 目标 <3s |

**性能碾压倍数（1GB 冷启动）：**
- 比 LogViewPlus 快 **100 倍**（60s → 0.6s）🔥
- 比 Daucloud 快 **12 倍**（7s → 0.6s）
...
- **性能碾压：比 LogViewPlus 快 100 倍，比 Daucloud 快 12 倍**
- 表格视图，字段可见可过滤
