...
|------|-------|-------------|-------------------|----------|
| **1GB 冷启动索引** | ~800ms | **60,000ms** | **15,000ms** | **584ms** ✅ |
| **1GB 热启动索引** | ~150ms | ~5,000ms | ~3,000ms | **88ms** ✅ |
| **1GB 热搜索** | ~150ms | ~200ms | ~500ms | **69ms** ✅ |
| **JSONL 字段过滤** | N/A | ~10,000ms | ~5,000ms | **74ms** ✅ |
| **10GB 文件打开** | ~3s | ~600s | ~150s | 目标 <3s |

**性能碾压倍数（1GB 冷启动）：**
- 比 LogViewPlus 快 **100 倍**（60s → 0.6s）
- 比 Daucloud 快 **25 倍**（15s → 0.6s）
...
- **VS Code 插件 Daucloud 支持 JSONL，但1GB 冷启动耗时 15 秒**
...
- **性能碾压：比 LogViewPlus 快 100 倍，比 Daucloud 快 25 倍**
- 表格视图，字段可见可过滤
...
| Daucloud (VS Code) | 15 秒 | 0.6 秒 | **25x** |
| klogg | 0.8 秒 | 0.6 秒 | **1.4x** |

**宣传语**：
> "LogViewPlus 需要 1 分钟，我们只需 0.6 秒"
> "VS Code 插件需要 15 秒，我们只需 0.6 秒"
