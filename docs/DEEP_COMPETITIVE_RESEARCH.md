...
|------|-------|-------------|-------------------|----------|
| **1GB 冷启动索引** | 未实测 | **60,000ms** | **7,000ms** | **584ms** ✅ |
| **1GB 热启动索引** | 未实测 | ~5,000ms | ~500ms | **88ms** ✅ |
| **1GB 热搜索** | 未实测 | ~200ms | ~500ms | **69ms** ✅ |
| **JSONL 字段过滤** | N/A | ~10,000ms | ~500ms | **74ms** ✅ |
| **10GB 文件打开** | 未实测 | ~600s | ~70s | 目标 <3s |
...
**核心结论：丹青日志的「JSONL 列化（桌面端唯一）+ 打开速度碾压 LogViewPlus」双优势，在桌面日志查看器市场构成差异化，值得全力推进。**
