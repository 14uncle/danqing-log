...
1. **性能问题**：klogg 用户抱怨大文件搜索慢（DEEP §3.1 #6：「10GB file search takes minutes」，12 反应）；我们 1GB 搜索 115ms（2026-09-15 复测，默认大小写不敏感）。**对 klogg 禁止速度声称**（mmap 同构，其速度未实测）
...
| 问题 | 用户原话 | 频率 |
|------|----------|------|
| 性能差 | "116KB file takes 15 seconds to search"（⚠️ 来源链接未留存，仅作氛围证据，勿当性能主张引用；对外用本机实测：1GB 冷启动 60 秒） | 高 |
...
2. **价格敏感**：LogViewPlus $45 个人被接受、$95 企业被嫌贵 → 我们的锚应低于 $45
   （2026-09-12 定 $29 个人 / $59 企业）
...
**铁证**：
- ✅ 性能测试：1GB 文件索引 92/94ms（热启动，2026-09-15 复测）
...
- ✅ 竞品缺口：klogg 无 JSONL；LogViewPlus 仅基础支持且 1GB 冷启动实测 60 秒（DEEP §5.1）——「独家」不成立
- ✅ 用户需求：klogg issues #387 要求 JSON 支持
- ✅ 市场验证：VS Code JSON 扩展有100万+下载量

**验证结论**：✅ 成立（差异化功能，市场需求明确；正确口径 = 「原生桌面、秒开的 JSONL 列化」，见 DEEP §5.1）
...
**铁证**：
- ✅ LogViewPlus $45 个人 / $95 企业，我们 $29 个人 / $59 企业
- ✅ 用户反馈 "$95 per seat is too expensive for small teams"
- ✅ 买断制，无订阅
- ⚠️ 2026-09-12 修正：原写「我们 $45」是按「功能对齐 LogViewPlus」定的；四张对比图逐项核实后
  确认真缺四项（多文件/标签、等级直方图、导出、会话模板），$45 同价无辩解空间 → 改 $29/$59
...
**丹青产品线定价**（2026-09-12 重裁）：

| 产品 | 定价 | 目标用户 |
|------|------|----------|
| 丹青番茄钟 | 免费 (GitHub) / Freemium (MS Store) | 个人用户 |
| 丹青日志 | 免费层 $0 / 付费层 $29 个人 · $59 企业 | 开发者/运维 |
| 未来产品 | 待定 | - |

**定价策略**（取代原 $45/$95；裁决与理由见 `ROADMAP-v1x.md` §三）：

- **免费层 $0 = 看懂**：单文件、mmap 秒开、虚拟滚动、正则搜索高亮、tail、JSONL 列化+嵌套、
  编码检测、书签、等级直方图。GitHub 永久免费开源 —— 这是获客引擎（对标 klogg 抢「停更 4 年」接盘流量）。
- **付费层 $29 个人 / $59 企业 = 批量/留存/交付**：多文件时间戳合并、字段分析、导出、工作台会话持久化。
- **为什么不是 $45**：$45 恰是 LogViewPlus 的价格，同价会让用户带着「它有的我都有」的预期进来，
  而我们真缺四项（多文件/标签、等级直方图、导出、会话模板）—— 预期必然落空。$29 让价格与事实一致。
- **渠道分层**：GitHub 走「永久免费 + 付费层买断」；MS Store 走「Store 内置 trial → 买断」，
  且**试用过期降级到免费层，不变砖**。
- **买断制**：无订阅，降低用户顾虑（Dadroit 按大小收订阅正是其口碑裂缝，反向操作即差异化）。
...
- 专业日志查看器市场空白
- 性能+功能+维护的差异化机会
- 竞品 $45/$95 区间被市场接受 → 我们的 $29/$59 有下探空间
