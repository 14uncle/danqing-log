...
> **2026-09-14 部分翻案**: 两条已拍板决策被 `SPEC-selection-copy.md` 推翻 ——
> ① 双击 = 空白分隔 token → 改为**混合连接器规则 + 中文逐字**;
> ② 「原始模式 Ctrl+C 只认文本选区」→ 改为**三级优先链** (文本选区 > 单元格 > 行选中,
> 两模式统一行兜底)。本文档其余部分 (选区模型 / 50 万行超限 / 焦点链路 / Esc 语义)
> 仍然有效。
