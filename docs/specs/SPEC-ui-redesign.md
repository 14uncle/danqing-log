...
> **五模块闭环后另立四条待裁**（浅色三支「面」token / 浅色语义色 sub-AA /
> 浅色两条着色路径分叉 / 框架 `composite_over` 的混合空间）——
> **正文在 `tasks/todo-open-decisions.md`（D1–D4）**, 本处不抄数字, 免得两处漂。
> **D1 已裁已修**（`surface_variant` 改按暗色台阶取, 框架 `0d91ed7`）;
> D2 / D3 / D4 待裁。
