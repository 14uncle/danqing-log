...
- [ ] 人工过目 (留用户: 进度前进观感 + 中途 Ctrl+O 取代体感)
...
- [x] 三件套绿
- [ ] 人工: spec 成功判据清单全走一遍 (docs/specs/SPEC-async-open.md) —— 见 T6 遗留项
...
- [ ] spec 成功判据人工项 (下「遗留人工验收」)
- [x] 三件套绿

**遗留人工验收** (spec 成功判据剩余项):
- [ ] 10GB 冷开全程可响应 (4GB 已证机制; 量级复核: genlog/coldgen 造 10GB 实机)
- [ ] 索引中 Ctrl+O 开另一文件 → 旧 job 取消 (构造保证 + 单测, 人工确认体感)
- [ ] 索引中关闭窗口 → 干净退出 (任务管理器读数)
- [ ] 轮转重建期间旧内容可见可滚 (copytruncate/create 两流派各一次)
- [ ] Loading 文案观感定档 (MiB 格式/百分比, plan 开放项)
