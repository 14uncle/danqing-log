...
- [ ] **T2: 并行计数 `count_levels` + 索引零回归实测** ⚠️ **高风险, 决定后续形态**
  ...
  - Acceptance: 并行结果 == 串行结果 (对拍单测);
    ...
    **logbench 索引耗时与基线 77 / 88ms 一致** (计数在索引趟外, 该数字不得变动)
...
- [ ] 6 桶之和 == 总行数 (对拍绿)
- [ ] 并行 == 串行 (对拍绿)
- [ ] 1GB 明文 / JSONL 计数墙钟实测落档
- [ ] logbench 索引 77 / 88ms **未变动**
- [ ] **墙钟 > 150ms → 停下复核**: 是否改判「先显示文件、计数随后补入」
      (该改判引入侧栏「计算中」态与新的作业失效链, 属 Ask-first, 需用户复核后再动)
