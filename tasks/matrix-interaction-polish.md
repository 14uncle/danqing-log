...
- [ ] **右键**: 行上右键 (**P29**)
- [ ] **其它**: 打开失败时的底栏 (**P27**) / Ctrl+L 关侧栏后找不回来 (**P37**, 改从 ⚙ → 常规页找) /
      托盘菜单 (**P38**, 改判不做 —— 项上**不该**有快捷键文字)
- [ ] **新增 (M4/M5 落地后)**: 抓滚动条拖 (**P30/T17**, 含横条与 hover 加深) /
      复制成功后底栏回执 **4 秒消退** (T18, **时长本身待核**: 太短没看见? 太长碍事?) /
      Ctrl+F **连按两次** (`/` 只在原始模式) —— 第二次应是**全选**而不是清空 (T21) /
      设置卡常规页的**侧栏开关**与 Ctrl+L 是否互相跟得上 (T20)
- [ ] **新增 (review 轮)**: **Ctrl+O 打开文件后, 当前行应当已经高亮**且 ↑↓ 看得见在动
      (T14 把高亮改成跟随焦点后, 打开文件若没人持焦就是「一行没选中 + 按了看不到动」,
      故补了「打开即把焦点送进列表」) / **点进搜索栏 → 日志里的高亮会消失, 点回列表即恢复**
      (这是 T14 的设计, **看到不算 bug**) / 设置卡开着时**卡内键盘要能用**:
      Tab 到侧栏开关按 Space 能切、主题下拉按 ↓ 能导航、关闭钮按 Enter 能关
      (review 抓到过一个 Critical 正是这条全挂) / 设置卡里横向**有没有文字顶出卡片右缘**
