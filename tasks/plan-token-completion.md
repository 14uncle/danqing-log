...
把它锁死 —— 那是**有意**设计, 让窗口底色透出）, 所以标题栏那条的颜色**就是**
`clear_color`。像素实测确认: 修后 y=2 全宽 `(25,25,32)`, 标题栏左侧 x<173
（logo + 标题）在 y=20 有 63 个底色像素。

> **留档: 我在这条上连错两次**。最初推测「亮带 = `clear_color`」**对**; 随后自纠
> 说「不是它」**错** —— 因为三个采样点 x=300/900/1700 **全落在过滤框内部**
> （框的范围 173..1796）, 把过滤框的灰当成了标题栏的灰。
> **教训: 采样点必须跨控件边界取; 三个点落在同一控件里等于没有对照组。**

**顺带照出来的（不归本模块）**: 过滤框 / 表头 / 斑马行三处都是
`DarkTheme::surface_variant()`（白 10%）, 经模块 1 的 linear 混合后显示成
`(93,93,94)` 而非设计意图的 `(48,48,54)` —— **约 2 倍**。那是模块 1 的已知代价,
归**模块 2 的 alpha 重校**（详见 SPEC §3 A1）。
