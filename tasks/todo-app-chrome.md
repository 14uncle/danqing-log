...
- [x] **A2: Bar 适配 embed 槽几何** ✅ 代码分析确认 A1 隐式完成 (TitleBar embed 机制把槽 area 传给 Bar; Hidden 零高跳过绘制)
  ...
  - 实测: Bar::layout 返回 constraints.max() 宽 (槽约束); paint/event 用 area 参数 (槽绝对坐标);
    input_area 以 area 为基准; Hidden→零高→TitleBar 跳过绘制; 56 lib+6 主测试绿, clippy 0

- [x] **A3: 标题串 + 窗口配置一致性** ✅ 编码 (人工验收待用户)
  - Acceptance: TitleBar.title = `丹青日志 POC [JSONL] — <file>`(或 文件名·模式); WindowConfig.title 同串;
    随 mode 带/不带 JSONL
  - Verify: 人工(任务栏与 TitleBar 一致; Ctrl+T 切模式)
  - Files: `src/main.rs` + 框架 `src/event.rs`/`src/app.rs`/`src/window/handler.rs`
  - 实测: 框架 App trait 加 `window_title()` (每帧查询, 变化时 set_title); WindowAction 加 SetTitle(String);
    产品 LogApp::make_title() 统一标题逻辑; view()/window_title() 共用; danqing 454 测试+danqing-log 62 测试绿, clippy 0

- [x] **A4: 键盘/焦点/IME 回归** ✅ 代码逻辑验证通过 (人工 IME 验收待用户)
  ...
  - 代码分析: Bar::handle_enter → ApplyFilter/ApplySearch; handle_escape → ClearFilter/CloseSearch;
    PageUp/Down 拦截 → ScrollRows; app_key_filter 拦 Ctrl+T; 其余转发 TextInput (Tab/方向键不冲突) ✅

- [x] **A5: 三件套 + 人工验收** ✅ 三件套绿 (人工验收待用户)
  - Acceptance: 三件套绿; 手动三键(最小/最大/还原/关闭)/拖拽移窗/双击最大化全过
  - Verify: `cargo fmt` + `cargo clippy --all-targets -- -D warnings` + `cargo test`; 人工
  - Files: —
  - 实测: danqing 454 测试+danqing-log 62 测试绿, clippy 0, release 构建成功;
    exe 在 `/.cargo-target/release/danqing-log.exe`

- [ ] **Checkpoint: 模块验收** ⏳ 三件套绿; 三键/拖拽/输入/IME/快捷键人工过; 进 review
