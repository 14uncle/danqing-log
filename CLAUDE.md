...
- 当前: **UI 改造五模块已闭环; v1.0 收尾是唯一主线** ——
  ...
  ② **v1.0 收尾** (原为并行线, 现为唯一主线):
     ...
     **(d) 隐私政策已落盘** (2026-09-13) → `docs/privacy-policy.md`。
     `runFullTrust` 会被问「是否收集个人信息」且**表单未必允许按事实选** (pomodoro 撞过);
     pomodoro 那份**至今只在 Partner Center 字段里、没落盘仓库** —— 我们落盘了。
     **一份覆盖两个渠道** (理由见下面当日条目); **提交时用 URL 不用贴文本**,
     避免「仓库改了、商店页还是旧版」的第二份副本
...
- 2026-09-13 (**上架物料 (1/3): 隐私政策落盘** `docs/privacy-policy.md`):
  ...
  **提交策略: 用 URL 不用贴文本** (指默认分支 `dev` 上这一份) —— 贴文本 = 第二份副本 =
  本仓反复犯的「写两处然后漂」。**我第一版推荐 URL 写的 `master` 是错的** (文件只在 `dev`,
  填 `master` 会 404), 已在附录里改成 `dev` 并加「提交前自己在浏览器打开确认能看」。
  **(d) 那个「是否收集个人信息」的单选我没替用户断言**: 按事实是「不收集」, 但
  **没证据说表单给不给这个选项**, 故写成判断而非结论, 并写明「别为了过关在『是』上硬扛 ——
  那会与政策正文自相矛盾」
