...
- **付费层（v1.x，收银台与首批功能腿已建成，销售通路待开业）**：免费层永久免费不变；
  付费层是**新增**的批量 / 留存 / 交付能力（$29 个人 / $59 企业买断，见
  [`docs/ROADMAP-v1x.md`](docs/ROADMAP-v1x.md)）。首批腿 = **字段分析**（数值分位数 /
  枚举分布）与**导出**（过滤结果 / JSON 美化 / CSV）；便携版 license key 离线校验不联网；
  购买链接开业后回填此处
...
| `Ctrl+E` | 导出…（付费层 —— 过滤结果 / JSON 美化 / CSV；免费态弹升级提示） |
| `Esc` | 关搜索栏 / 清过滤 / 关设置卡·升级提示·导出菜单 / 清选区与单元格选中 |
